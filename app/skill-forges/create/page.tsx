...
export default function CreateSkillForgePage() {
  return (
    <ProtectedRoute role="company">
      <CreateSkillForgeForm />
    </ProtectedRoute>
  );
}
...
function CreateSkillForgeForm() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!user) return;
      const { data } = await supabase.from("users").select("id, company_name").eq("id", user.id).single();
      if (data) { setCompanyId(data.id); setCompanyName(data.company_name || "Your Company"); }
    })();
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!companyId) { setError("Company profile not found."); setLoading(false); return; }
    if (!form.title || !form.description || !form.requirements) { setError("Title, description, and requirements are required."); setLoading(false); return; }
    const { error: insertError } = await supabase.from("skill_forges").insert({
      title: form.title, description: form.description, category: form.category,
      difficulty: form.difficulty, requirements: form.requirements,
      deadline: form.deadline ? new Date(form.deadline).toISOString() : null,
      reward_clout: Number(form.reward_clout), company_sponsor_id: companyId, status: "active",
      skills_required: form.skills_required.split(",").map(s => s.trim()).filter(Boolean),
      estimated_time: form.estimated_time || null, submission_format: form.submission_format || null,
        ...
        <div className="csf-layout" style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 360px", gap:24, alignItems:"start" }}>
          ...
          <div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {step === 4 && (
                <div className="csf-step-anim">
                  <FormCard title="Step 4 — Review & Launch" subtitle="Double-check everything before your Forge goes live.">
                    ...
                    <div style={{ background:"linear-gradient(135deg,#f0f9ff,#eef2ff)", borderRadius:18, padding:"20px", border:"1px solid #bfdbfe" }}>
                      <h3 style={{ margin:"0 0 14px", fontSize:15, fontWeight:900, color:"#0f172a" }}>What happens after you launch?</h3>
                      <div style={{ display:"grid", gap:10 }}>
                        {[
                          { icon:"🌐", text:"Your Forge goes live instantly on the SkillForges public page." },
                          { icon:"📩", text:"Candidates browse, read requirements, and submit a SkillCapsule." },
                          { icon:"📊", text:"You receive and review all capsule submissions in your Foundry." },
                          { icon:"🏆", text:"Top performers earn Clout points — your brand gets visibility." },
                        ].map((item, i) => (
              ...
              <div style={{ display:"flex", gap:12, marginTop:20, flexWrap:"wrap" }}>
                ...
                {step < 4
                  ...
                  : <button type="submit" className="csf-btn" disabled={loading}
                      style={{ background: loading ? "#94a3b8" : "linear-gradient(135deg,#10b981,#059669)", color:"white", border:"none", padding:"13px 28px", borderRadius:16, fontWeight:900, fontSize:16, cursor: loading ? "not-allowed" : "pointer", flex:1, boxShadow: loading ? "none" : "0 8px 24px rgba(16,185,129,.35)", display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
                      {loading
                        ? <><div style={{ width:18, height:18, borderRadius:"50%", border:"3px solid rgba(255,255,255,.3)", borderTopColor:"white", animation:"spin 0.8s linear infinite" }} /> Launching…</>
                        : "🚀 Launch Skill Forge Now"
                      }
                    </button>
                }
              </div>
